...
AB_simulation.jl を実行するとシミュレーションとプロットが行われる。

パッケージ StatsPlots, StatsBase, Random を用いる。自身の環境に追加されたし

プロットするグラフの保存先のパスは、README.mdがおいてあるディレクトリでファイルを実行することを想定している

## 1.1. RAM

自分の環境下では、ほとんどミニマムなシミュレーション条件

$J, O, N, Oin, STIME, SEASON = 600, 10, 3, 1, 60, 5$

において1.5GB程度、メモリを占有するようだ。ということで、小規模なシミュレーションを行うなら、メモリは最低4GB、推奨8GB以上。

速度は条件やCPU性能によるだろうが、少なくともコンパクトな条件下では待てないほどの遅さじゃない
...
- 要求賃金率上昇率≒名目政府支出増加率
  - ここで言う要求賃金率上昇率は、家計全体の平均の話であり、また事実上、失業率の単調減少関数となっている
- 名目政府支出の増加率が高いと、失業率は下がり、インフレ率は上がる傾向がある。名目需要の増加は比較的硬直的な価格のもとで実質需要を増加させて失業率を下げる。低い失業率が支払い賃金を増加させ価格転嫁されることでインフレ率が上がる

# 6. 修正・変更・拡張の方向性

## 6.1. 問題
- Oin=2とすると、G_func()のG_potential = max.(0.0, G_calc_item .- β4) .* k[os,tm].^β5で、G_calc_itemとk[os,tm]の配列の大きさが異なる
...
## 6.3. 実施を検討していること
- 消費性向を可処分所得と資産と前期平均消費の関数とする。現状、可処分所得と金融資産保有額で決めているが、資産家の家計が消費性向が低いことを記述したい
