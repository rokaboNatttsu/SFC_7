...
- 公債と公債利回りの追加を検討
  - 公債利回りと民間への貸出金利の適応的連動 $r_L=(1-\lambda_r)r_{L-1}+\lambda_r \epsilon r_{GB}$ を検討。
- 労働者と投資家の分割を検討
...
- $G^m=(1+\beta)G_{-1}^m$ ここ考え中。支出先の切り替え・発注量増減をどんなアルゴリズムにするのか。
  - $\sum G^m=(1+\beta)\sum G_{-1}^m$ の束縛条件は満たす必要がある
- 
- $g=\frac{G}{p}$
- $C=\alpha_1(W_{-1}-T_a-T_i-r_L L_{-1}+P_{h-1}+S_{-1}) + \alpha_2(E_{h-1}+M_{-1}-L_{-1})$
- $c=\frac{C}{p}$
