...
- [2. エージェント・ベース・モデル](#2-エージェントベースモデル)
  ...
  - [2.3. エージェント自身のフローの一貫性（貸借対照表及びキャッシュ・フロー計算書のようなもの）の表](#23-エージェント自身のフローの一貫性貸借対照表及びキャッシュフロー計算書のようなものの表)
  - [2.4. エージェントのストックの一貫性（バランスシート）の表](#24-エージェントのストックの一貫性バランスシートの表)
  - [2.5. 部門内のストックとフローの一貫性](#25-部門内のストックとフローの一貫性)
  - [2.6. エージェントのストックとフローの一貫性](#26-エージェントのストックとフローの一貫性)
...
## 2.3. エージェント自身のフローの一貫性（貸借対照表及びキャッシュ・フロー計算書のようなもの）の表
...
## 2.4. エージェントのストックの一貫性（バランスシート）の表
|          |        家計        |        企業        |        銀行        |   統合政府    |
| :------- | :----------------: | :----------------: | :----------------: | :-----------: |
| 資本     |                    |       $+K^o$       |                    |               |
...
| 銀行株式 | $+\sum_o F_h^{ol}$ |                    |       $-F^n$       |               |
| 現金     |                    |                    |       $+H^n$       | $-\sum_n H^n$ |
| 純資産   |     $-NW_h^l$      |     $-NW_f^o$      |     $-NW_b^n$      |    $-NW_g$    |
| 合計     |        $0$         |        $0$         |        $0$         |      $0$      |

## 2.5. 部門内のストックとフローの一貫性

## 2.6. エージェントのストックとフローの一貫性
